...
// TestResponseOutboundCapturesArgumentsFromDoneEventWithoutDeltas reproduces the
// codex "stalls after one spoken sentence" bug. Some Responses upstreams (observed:
// anyrouter's gpt-5.6 proxy at low reasoning effort) deliver the COMPLETE tool-call
// arguments in a single response.function_call_arguments.done event with NO
// preceding function_call_arguments.delta chunks. octopus had no handler for that
// event and dropped the arguments, so codex received a function call with empty
// arguments it could not execute. The done event must surface the arguments.
func TestResponseOutboundCapturesArgumentsFromDoneEventWithoutDeltas(t *testing.T) {
	outbound := &ResponseOutbound{}
	feed := func(payload string) *model.InternalLLMResponse {
		resp, err := outbound.TransformStream(context.Background(), []byte(payload))
		if err != nil {
	...
	// name arrives on output_item.added; no arguments yet
	feed(`{"type":"response.output_item.added","output_index":1,"item":{"id":"fc_1","type":"function_call","call_id":"call_1","name":"exec"}}`)

	// the COMPLETE arguments arrive only on the done event — no delta chunks at all
	done := feed(`{"type":"response.function_call_arguments.done","output_index":1,"item_id":"call_1","arguments":"{\"command\":\"pwd\"}"}`)
	if done == nil || len(done.Choices) != 1 || done.Choices[0].Delta == nil || len(done.Choices[0].Delta.ToolCalls) != 1 {
		t.Fatalf("expected function_call_arguments.done to emit the tool-call arguments, got %#v", done)
	}
	tc := done.Choices[0].Delta.ToolCalls[0]
	if tc.Index != 1 || tc.ID != "call_1" {
		t.Fatalf("expected done chunk to carry the tool-call index/id, got %#v", tc)
	}
	if tc.Function.Arguments != `{"command":"pwd"}` {
		t.Fatalf("expected the full arguments to be surfaced from the done event, got %q", tc.Function.Arguments)
	}
}

// TestResponseOutboundDoneEventDoesNotDuplicateStreamedArguments guards the same
// handler against double-emitting when an upstream sends BOTH incremental deltas and
// a final done carrying the full arguments: the done must contribute only the
// not-yet-streamed suffix (here, nothing).
func TestResponseOutboundDoneEventDoesNotDuplicateStreamedArguments(t *testing.T) {
	outbound := &ResponseOutbound{}
	feed := func(payload string) *model.InternalLLMResponse {
		resp, err := outbound.TransformStream(context.Background(), []byte(payload))
		if err != nil {
	...
	feed(`{"type":"response.output_item.added","output_index":0,"item":{"id":"fc_1","type":"function_call","call_id":"call_1","name":"exec"}}`)
	feed(`{"type":"response.function_call_arguments.delta","output_index":0,"delta":"{\"command\":"}`)
	feed(`{"type":"response.function_call_arguments.delta","output_index":0,"delta":"\"pwd\"}"}`)
	done := feed(`{"type":"response.function_call_arguments.done","output_index":0,"arguments":"{\"command\":\"pwd\"}"}`)
	if done != nil {
		for _, ch := range done.Choices {
			if ch.Delta == nil {
				continue
			}
			for _, tcc := range ch.Delta.ToolCalls {
				if tcc.Function.Arguments != "" {
					t.Fatalf("done event must not re-emit already-streamed arguments, got %q", tcc.Function.Arguments)
				}
			}
		}
	}
