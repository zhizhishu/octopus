...
func DBExportAll(ctx context.Context, includeLogs, includeStats bool) (*model.DBDump, error) {
	...
	if err := conn.Find(&d.RedeemCodes).Error; err != nil {
		return nil, fmt.Errorf("export redeem_codes: %w", err)
	}
...
func DBImportIncremental(ctx context.Context, dump *model.DBDump) (*model.DBImportResult, error) {
	...
	err := conn.Transaction(func(tx *gorm.DB) error {
		// base tables
		if n, err := createDoNothing(tx, dump.Users); err != nil {
		...
		if n, err := createDoNothing(tx, dump.RedeemCodes); err != nil {
			return fmt.Errorf("import redeem_codes: %w", err)
		} else {
			res.RowsAffected["redeem_codes"] = n
		}
