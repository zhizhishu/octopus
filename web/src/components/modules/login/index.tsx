...
type LoginMode = 'user' | 'apikey' | 'register';
...
export function LoginForm({ onLoginSuccess }: { onLoginSuccess?: () => void }) {
  ...
  const isPending = loginMutation.isPending || registerMutation.isPending || apiKeyLoginMutation.isPending
  const inviteRequired = registrationOptions?.invite_required ?? true
        ...
        <Tabs value={mode} onValueChange={handleModeChange}>
          <TabsList className="flex p-1 bg-muted rounded-2xl">
            <TabsHighlight className="rounded-xl bg-background shadow-sm">
              ...
              <TabsHighlightItem value="register" className="flex-1">
                <TabsTrigger
                  value="register"
                  className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-sm font-medium transition-colors data-[state=active]:text-foreground data-[state=inactive]:text-muted-foreground data-[state=inactive]:hover:text-foreground"
                >
          ...
          <form onSubmit={handleSubmit} className="space-y-6 pt-2">
              ...
              <TabsContent value="register" className="space-y-6">
                ...
                <Field>
                  ...
                  <Input
                    id="invite-code"
                    type="text"
                    placeholder={inviteRequired ? t('inviteCodeRequiredPlaceholder') : t('inviteCodeOptionalPlaceholder')}
                    value={inviteCode}
                    onChange={(e) => setInviteCode(e.target.value)}
                    required={mode === 'register' && inviteRequired}
                    disabled={isPending}
                  />
                  <FieldDescription>
                    {inviteRequired ? t('inviteRequiredHint') : t('inviteOptionalHint')}
                  </FieldDescription>
                </Field>
              </TabsContent>
            ...
            <Button type="submit" disabled={isPending} className="w-full">
              {isPending ? t('button.loading') : (mode === 'register' ? t('button.register') : t('button.submit'))}
            </Button>
          </form>
        </Tabs>
      </div>
