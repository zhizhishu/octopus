        ...
        <div className="flex h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] max-w-full min-h-0 flex-col md:max-w-4xl">
            ...
            <MorphingDialogDescription className="flex-1 min-h-0 overflow-hidden">
                <GroupEditor
                    ...
                    onSubmit={({ name, match_regex, mode, mode_locked, first_token_time_out, session_keep_time, max_concurrent, rpm_limit, members }) => {
                        ...
                        createGroup.mutate(
                            { name, mode, mode_locked: mode_locked ?? false, match_regex: match_regex ?? '', first_token_time_out: first_token_time_out ?? 0, session_keep_time: session_keep_time ?? 0, max_concurrent: max_concurrent ?? 0, rpm_limit: rpm_limit ?? 0, items },
                            {
