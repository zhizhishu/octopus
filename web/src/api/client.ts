...
function summarizeHTMLResponse(value: string): string | undefined {
...
function sanitizeErrorMessage(value: string): string {
    ...
    return summarizeHTMLResponse(trimmed) ?? trimmed;
}
    ...
    if (!response.ok) {
        const error: ApiError = {
            ...
            message: (data && typeof data === 'object' && 'message' in data && typeof data.message === 'string')
                ? sanitizeErrorMessage(data.message)
                : (typeof data === 'string' ? sanitizeErrorMessage(data) : response.statusText),
        };
