...
> [!NOTE]
> **Octopus**：由 `zhizhishu/octopus` 维护，主打缓存稳定性修复、自动模型分组、模型单测/并发测试、缓存率观测、日志审核、真实多用户管理、人性化月卡/兑换码、New API 活跃用户迁移与后台 Job 迁移页、方案组模型映射、路由/分组配置引导、独立 API Key、提示词管理、Gemini / OpenAI / Anthropic 协议互转、CPA/CLIProxyAPI Responses 兼容、API Key 用法引导、`models.dev` 价格库入口和 GHCR 镜像发布。完整说明和截图见 [README_FUTURE.md](README_FUTURE.md)。
>
