...
- **日志详情与状态审计**：
  ...
  - 日志详情标出入口端点/协议和原始请求路径，例如 `chat`、`responses`、`messages`，方便确认客户端调用协议。
  - 支持按用户名、API Key、模型名、渠道名、端点、路径、会话 Key、错误码等跨字段**模糊搜索**，以及按端点协议、厂商系列、具体模型的组合**精准筛选**。
  - **挂起请求自动救援**：针对触发拦截的临时失败请求，网关默认由机器按访问计划自动救援（轮询/优先填充，多轮自动重读最新渠道和 Key，指数退避并维持客户端连接保持）；人工选择渠道重试为「立即覆盖」，非必需步骤。
